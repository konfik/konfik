...
export const PackageJsonKonfik = KonfikFactory<PackageJsonRaw>()({
  ...
  toString: (config) => {
    // Move `name` to the top of generated `package.json`
    if (config.name) {
      const { name, ...restConfig } = config
      config = { name, ...restConfig }
    }

    config = {
      ['_generatedByKonfik' as any]: 'NOTE: This file was generated by Konfik and should not be edited directly',
      ...config,
    }
