...
export const readDir = (dirPath: string): T.Effect<OT.HasTracer, ReadDirError | DirNotFoundError, string[]> =>
  pipe(
    T.tryCatchPromise(
      () => fs.readdir(dirPath),
      (error: any) => {
        if (error.code === 'ENOENT') {
          return new DirNotFoundError({ dirPath })
        } else {
          return new ReadDirError({ dirPath, error })
        }
      },
    ),
    OT.withSpan('readDir', { attributes: { dirPath } }),
  )

export const readFileAsBuffer = (filePath: string): T.Effect<OT.HasTracer, ReadFileError | FileNotFoundError, Buffer> =>
  OT.withSpan('readFileAsBuffer', { attributes: { filePath } })(
    T.tryCatchPromise(
...
export const rename = (oldPath: string, newPath: string): T.Effect<OT.HasTracer, RenameError, void> =>
  OT.withSpan('rename', { attributes: { oldPath, newPath } })(
    T.tryCatchPromise(
      () => fs.rename(oldPath, newPath),
      (error) => new RenameError({ oldPath, newPath, error }),
    ),
  )
...
export class DirNotFoundError extends Tagged('node.fs.DirNotFoundError')<{ readonly dirPath: string }> {}

export class ReadDirError extends Tagged('node.fs.ReadDirError')<{ readonly dirPath: string; readonly error: any }> {}

export class RenameError extends Tagged('node.fs.RenameError')<{
